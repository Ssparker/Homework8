...
public class StringRandomizer {
    ...
    public static void addString(String value) {  //value.matches regex
        if (value.matches(".*\\d+.*")) { //  checks to see if there are numbers anywhere in this string
            System.out.println("Value with numbers encountered. BOOOOOOO!");
        } else {
            if (value.matches("([AEIOU]).*")) { // . checking first index and stops at .
